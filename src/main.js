...
let nInterId; let focusTick = 0;
let isGoogleEnabled = true;
const dbName = "safari_homepage";
const dbVersion = 2;
let db;
let updateQuery; let getQuery;
let suggestion = "";
let lastKey;
...
$(document).ready(() => {
  ...
  $(".settingButton.add").click(() => {
    ...
    const node = $(`<a href="${element.link}" title="${element.title}" class="siteButton"><i></i><img src="${element.img}" class="siteIcon">
  ...
  $(".settingButton.image").click(() => {
    const term = $("[data-key='shortcutTitle']").val();
    if (term.length == 0) {
      return;
    }
    ...
    fetch(url)
        ...
        .then((data) => {
          if (data.results) {
            const parent = $(".settingItem.full.image");
            parent.empty();
  ...
  if (window.config.shortcuts) {
    ...
    window.config.shortcuts.map((element) => {
      const template = `<a href="${element.link}" title="${element.title}" class="siteButton"><i></i><img src="${element.img}" class="siteIcon">
                        <div class="siteTitle">${element.title}</div></a>`;
  ...
  if (window.config.source === "local") {
    const id = Math.floor(Math.random() * window.config.count) + 1;
...
const addListener = (node) => {
  node.on("click", clickListener)
      .on("mousedown", (e) => {
        ...
        const drg_h = $obj.outerHeight();
        const drg_w = $obj.outerWidth();
        const pos_y = $obj.offset().top + drg_h - e.pageY;
        const pos_x = $obj.offset().left + drg_w - e.pageX;
        $obj.parents().on("mousemove", (e) => {
          const idx = $obj.index(".siteButton:not(.add)");
          const top = e.pageY + pos_y - drg_h;
          const left = e.pageX + pos_x - drg_w;
          $obj.offset({
            top: top,
            left: left,
          }).on("mouseup", () => {
        ...
        const idx = $obj.index(".siteButton:not(.add)");
        const offset = $(".siteButton.moveLeft").length - $(".siteButton.moveRight").length;
        const element = window.config.shortcuts[idx];
...
const clickListener = (e) => {
  if (e.target.nodeName == "I") {
    const index = $(e.currentTarget).index(".siteButton:not(.hide)");
...
const requestImage = (callback) => {
  if (window.config.source === "bing") {
    const url = "https://www.bing.com/HPImageArchive.aspx?format=js&idx=" + Math.round(Math.random() * 10) + "&n=1";
    fetch(url)
        .then((res) => res.json())
        .then((data) => {
          callback(data);
...
const textChange = (e) => {
  ...
  const query = $(obj).val().toLocaleLowerCase();
  ...
  getQuery(query, (data) => {
    if (data && data.value) {
      ...
      const str = $(obj).val() + suggestion.slice(query.length);
      $(obj).val(str);
...
const enterSearch = (e) => {
  ...
  if (code == 13 && $("#textInput").val()) {
    if (updateQuery) {
      const query = $("#textInput").val().toLocaleLowerCase();
      updateQuery(query);
    }
